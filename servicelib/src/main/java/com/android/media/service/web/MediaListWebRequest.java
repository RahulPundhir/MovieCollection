 ...
 *
 */
public class MediaListWebRequest {
    public static final String LIST_PARAMETER = "s=";
    ...
    public MediaListWebRequest(Context context) {
    ...
    public class WebRequest extends AsyncTask<String, Void, Void> {
        MediaListResponse cachedData;
        MediaListResponse mResponse;
        ...
        protected void onCancelled() {
            super.onCancelled();
            Log.i("MediaWebRequest", "Cancelled");
        }
    }
     ...
     * @throws Exception
     */
    public MediaListResponse parse(InputStream in) throws Exception {
        MediaListResponse data = fromJSON(MediaListResponse.class, in);
        return data;
    }
     ...
     * @return
     */
    public static MediaListResponse fromJSON(Class clazz,
                                             final InputStream in) {
        MediaListResponse data = null;
        try {
    ...
    private String createURL(String param) {
        if (param.contains(WebConstants.SPACE)) {
            param = param.trim();
            param = param.replaceAll(WebConstants.SPACE, WebConstants.REPLACEMENT);
        }
        return WebConstants.SERVER_URL + LIST_PARAMETER + param + WebConstants.PLOT_SHORT_R_JSON;
    }
     ...
     * @return
     */
    private MediaListResponse getCachedData() {
        mCacheDB.open();
        ...
        if (entity != null) {
            ...
            try {
                ...
                MediaListResponse data = parse(stream);
                return data;
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public interface ServiceCallback {
        void onSuccess(MediaListResponse data);

        void onError(String error);

        void onNetworkError(MediaListResponse data, String error);
    }
}
